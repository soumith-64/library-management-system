...
if (isset($_POST["submit"])) {
    ...
    $stmt = $link->prepare("INSERT INTO newa (book_name,photo,book_Urls) VALUES (?,?,?)");

    $stmt->bind_param('sss', $book_name , $imagepath,$book_Urls);
...
.new{
    ...
    <form action="" method="post" enctype="multipart/form-data">
        <table class="inputs">
            <tr>
                <td>
                    <label for="book_name" style="font-weight: bold;">Book Name</label>
                    <input type="text" id="book_name" class="form-control" name="book_name" placeholder="Book Name" required="">
                    <label for="">Book Url</label>
                    <input type="text" name="book_Urls" class="form-control" placeholder="Book Url">
    ...
    <!-- Rest of your table and PHP code for displaying existing books -->
</div>
...
<table id="myTable" class="table table-bordered border-primary">
    <thead class="thead-light font-weight: bold;">
        <tr>
            <th>Photo</th>
            <th>Book Name</th>
            <th style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">Book Urls</th>
            <th>Edit book</th>
            <th colspan="1">Delete book</th>
        </tr>
    </thead>
    <tbody style="color:darkgreen; font-weight: bold;">
        <?php
        $res = mysqli_query($link, "select * from newa");
        while ($row = mysqli_fetch_array($res)) {
            echo "<tr>";
            echo "<td>";
            $photo = htmlspecialchars($row['photo']); // sanitize output
            echo '<img src="' . $photo . '" height="100" width="100" alt="Book Image">';
            echo "</td>";
            echo "<td>";
            echo $row["book_name"];
            echo "</td>";
            ...
            echo "</td>";
            echo "<td>";
            ?><a href="new.php?book_name=<?php echo $row["book_name"]; ?>"><button class="edit-button">
            ...
            echo "</td>";
            echo "<td>";
            ?><a href="delete-new.php?book_name=<?php echo $row["book_name"]; ?>" onclick="return checkdelete ()"><button class="buttons">
            <svg
...
$(document).ready(function () {
    ...
    $.fn.dataTable.ext.search.push(
        function (settings, data, dataIndex) {
            ...
            var books_price = parseFloat(data[7]) || 0; // use data for the return_date column

            if (
                (isNaN(min) && isNaN(max)) ||
                (isNaN(min) && books_price <= max) ||
                (min <= books_price && isNaN(max)) ||
                (min <= books_price && books_price <= max)
            ) {
                return true;
            }
            return false;
