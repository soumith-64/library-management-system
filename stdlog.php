...
if (isset($_COOKIE['session_token'])) {
    ...
    if (isset($_SESSION["student"])) {
        ...
        <script type="text/javascript">
            window.location="user/student/dashboard.php";
        </script>
        <?php
        exit();
    }
...
<html lang="en">
  <head>
    ...
    <style>
        ...
        @keyframes stroke {
            0% {
                fill: rgba(147, 255, 227, 0); stroke: rgba(0, 0, 0, 1);
                stroke-dashoffset: 25%; stroke-dasharray: 0 50%; stroke-width: 2;
            }
            70% { fill: rgba(147, 255, 227, 0); stroke: rgba(0, 0, 0, 1); }
            80% { fill: rgba(147, 255, 227, 0); stroke: rgba(0, 0, 0, 1); stroke-width: 3; }
            100% {
                fill: rgba(147, 255, 227, 1); stroke: rgba(0, 0, 0, 0);
  ...
  <body>
   <div class="container">
    <form action="" method="post">
        ...
        <a href="index.php" class="aa">Go Back</a>
    </form>
    <?php
    if (isset($_POST["login"])) {
        $count = 0;
        $res = mysqli_query($link, "SELECT * from std_registration where name='$_POST[username]' && password= '$_POST[password]' && status='yes'");
