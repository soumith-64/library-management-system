...
$timestamp = time();
$regno = $row['regno']; // <-- Add this

$token = hash_hmac('sha256', $username . $regno . $timestamp, $secret); // <-- Correct format

$profile_url = "https://wwislib.com/user/student/profilest.php?regno={$regno}&user=" . urlencode($username) . "&time={$timestamp}&token={$token}";
